...
interface IDaiPermitTokenV2 is IERC20 {
...
interface IERC4494TokenV2 is IERC721 {
 ...
 * @dev Version 2 tolerates a permit that was submitted before settlement only
 *      when the resulting allowance still supports the signed rescue.
 */
contract SafeExitPermitSettlementV2 is EIP712, ReentrancyGuard {
    ...
    error InsufficientSettlementAllowance(uint256 required, uint256 actual);
    ...
    error UnexpectedSettlementAllowance(uint256 expected, uint256 actual);
    error ZeroAddress();
    error ZeroAmount();
    ...
    constructor() EIP712("SafeExit Permit Settlement", "2") {}
    ...
    function settleERC2612(
        ...
        } else if (!_isNextNonce(permitNonce, currentNonce)) {
            revert InvalidPermitNonce(permitNonce, currentNonce);
        }

        uint256 allowance = erc20.allowance(owner, address(this));
        if (allowance != amount) revert UnexpectedSettlementAllowance(amount, allowance);
        _transferExactERC20(erc20, owner, destination, amount);
        _requireZeroAllowance(erc20, owner);
    ...
    function settleDaiPermit(
        ...
        IDaiPermitTokenV2 permitToken = IDaiPermitTokenV2(token);
        uint256 currentNonce = permitToken.nonces(holder);
        if (currentNonce == allowNonce) {
            permitToken.permit(
                holder,
                address(this),
                allowNonce,
        ...
        } else if (!_isNextNonce(allowNonce, currentNonce)) {
            revert InvalidPermitNonce(allowNonce, currentNonce);
        }

        IERC20 erc20 = IERC20(token);
        uint256 allowance = erc20.allowance(holder, address(this));
        if (allowance < amount) revert InsufficientSettlementAllowance(amount, allowance);
        _transferExactERC20(erc20, holder, destination, amount);

        if (erc20.allowance(holder, address(this)) != 0) {
            uint256 revokeNonce = allowNonce + 1;
            currentNonce = permitToken.nonces(holder);
            if (currentNonce != revokeNonce) revert InvalidPermitNonce(revokeNonce, currentNonce);
            permitToken.permit(
                holder,
                address(this),
                revokeNonce,
        ...
        _requireZeroAllowance(erc20, holder);
    ...
    function settleERC4494(
        ...
        IERC4494TokenV2 permitToken = IERC4494TokenV2(collection);
    ...
    function _isNextNonce(uint256 signedNonce, uint256 currentNonce) private pure returns (bool) {
        return signedNonce != type(uint256).max && currentNonce == signedNonce + 1;
    }
}
